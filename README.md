...
### 5. Static executables

Nixkell supports building statically-linked executables for your Haskell project for X86_64 Linux systems. Static executables are very small compared to the usual dynamically linked executables, and can be deployed and run on any Linux machine without installing any dependency libraries or even Nix. Building static executable for Haskell code is a very fussy process, but fortunately, Nix helps us put everything together easily, and Nixkell provides one simple command to build them.
...
`nix/scripts.nix` also provides the `build-static` command to build statically-linked executables. When run for the first time, it may take several hours to finish, but the subsequent builds finish in minutes. An X86_64 Linux machine with Nix is required for the build. If you want to run this on a different machine, you'll need to configure a remote (possibly virtual) X86_64 Nix builder.
...
To tweak things further you can add things to the manual section of `ourHaskell` in `packages.nix`, eg. say you want to remove version bound checks on `some-package`:
```
...
- `overlays-static.nix` - configures GHC and Haskell packages in nixpkgs to build static executables
...
- `static-deps.nix` - exposes the dependencies and build tools for static builds, to create Nix GC root for them
